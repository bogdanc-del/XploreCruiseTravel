...
export default function TestimonialCard({ testimonial }: TestimonialCardProps) {
  ...
  const dateStr = new Date(testimonial.created_at).toLocaleDateString(
    locale === 'ro' ? 'ro-RO' : 'en-GB',
    { month: 'short', year: 'numeric' },
  )

  return (
    <div className="bg-white rounded-xl border border-navy-100 p-6 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full">
      {/* Header: Stars + Date */}
      ...
      <blockquote className="text-sm text-navy-700 leading-relaxed flex-1 mb-4">
        <span className="text-gold-400 text-lg leading-none">&ldquo;</span>
        {testimonial.quote}
        <span className="text-gold-400 text-lg leading-none">&rdquo;</span>
      </blockquote>
      ...
      <div className="flex items-center gap-3 pt-4 border-t border-navy-50">
        ...
        <div>
          ...
          {testimonial.city && (
            <p className="text-xs text-navy-400">{testimonial.city}</p>
          )}
        </div>
      </div>
    </div>
