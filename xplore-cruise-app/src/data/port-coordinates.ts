 ...
 *
 * Maps 840+ cruise port names to [lat, lng] coordinates for RouteMap rendering.
 * Includes fuzzy matching for Romanian-format names ("Marseille, Franta").
 *
 * @module data/port-coordinates
 */

import { parsePortName, fuzzyMatchPort } from '@/lib/port-name-utils'

/** A geographic coordinate pair: [latitude, longitude] */
export type PortCoordinate = [latitude: number, longitude: number]

// ============================================================
// Port coordinates database (lat, lng) — comprehensive
// ============================================================

export const PORT_COORDS: Record<string, PortCoordinate> = {
  ...
  'Glacier Bay': [58.6658, -136.0000],
  'Icy Strait Point': [58.1339, -135.4294],
  'Hoonah': [58.1100, -135.4400],
  ...
  // ── Insula Catalina (California) ───────────────────────
  'Insula Catalina': [33.3494, -118.3267],

  // ── Special names (Romanian / landmarks) ───────────────
...
// ============================================================
// Sea-day patterns & coordinate aliases
// ============================================================

/** Patterns indicating a "sea day" (no port to plot) */
export const SEA_DAY_PATTERNS: string[] = [
  'pe mare', 'at sea', 'en mer', 'cruising', 'cruise', 'navigare', 'navigatie',
]

/** Additional name aliases for fuzzy coordinate matching */
export const COORD_ALIASES: Record<string, string> = {
  'athena': 'Athens',
...
// ============================================================
// Fuzzy port-name → coordinate lookup
// ============================================================

/**
 * Look up coordinates for a raw cruise port name.
 *
 * Resolution order:
 *  1. Skip sea-day patterns (returns null)
 *  2. Skip navigation notes (straits, cruising)
 *  3. COORD_ALIASES partial-match
 *  4. Exact match → city extraction → slash parts → case-insensitive (via `fuzzyMatchPort`)
 *  5. startsWith partial-match fallback
 *
 * @param rawName - Raw port name from API (e.g. "Marseille, Franta")
 * @returns Coordinates and display name, or null for sea days / unknown ports
 */
export function getPortCoordinates(
  rawName: string,
): { coords: PortCoordinate; displayName: string } | null {
  const lower = rawName.toLowerCase().trim()

  // Skip sea days
  if (SEA_DAY_PATTERNS.some(s => lower.startsWith(s))) return null
  // Skip navigation notes (but NOT canal cities like "Panama Canal" which we alias)
  ...
  for (const [alias, target] of Object.entries(COORD_ALIASES)) {
    if (lower.includes(alias) && PORT_COORDS[target]) {
      return { coords: PORT_COORDS[target], displayName: target }
    }
  }

  // 1. Use shared parsePortName + fuzzyMatchPort for structured matching
  const parsed = parsePortName(rawName)
  const matchedKey = fuzzyMatchPort(parsed, PORT_COORDS, COORD_ALIASES)

  if (matchedKey) {
    const displayName = parsed.city || matchedKey
    return { coords: PORT_COORDS[matchedKey], displayName }
  }

  // 2. Partial-match fallback — city name startsWith or is startedBy a known port
  const cityLower = parsed.cityLower
